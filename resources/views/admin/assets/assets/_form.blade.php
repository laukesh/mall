...
<div class="row">
    ...
    <div class="col-lg-4 col-md-6 mb-3">
        ...
        <input type="text" id="status" name="status"
               ...
               value="{{ old('status', $asset->status ?? '') }}"
               maxlength="50" placeholder="e.g. Active">
        @error('status')<div class="invalid-feedback">{{ $message }}</div>@enderror
    </div>

    <div class="col-lg-6 mb-3">
        <label for="conditions" class="form-label fw-semibold">Conditions</label>
        <textarea id="conditions" name="conditions" rows="3"
                  class="form-control @error('conditions') is-invalid @enderror"
                  maxlength="500">{{ old('conditions', $asset->conditions ?? '') }}</textarea>
