    ...
    <div class="card-body">
      <form action="{{ route('admin.contractor.update', $contractor->id) }}" method="POST">
        ...
        <div class="row">
          ...
          <div class="col-md-8 form-group" id="parent_contractor_group" style="{{ old('contractor_type', $contractor->contractor_type) === 'Sub Contractor' ? '' : 'display:none;' }}">
            ...
            <select name="parent_contractor_id" class="form-control">
              ...
              @foreach($mainContractors as $main)
                <option value="{{ $main->id }}" @selected(old('parent_contractor_id', $contractor->parent_contractor_id ?? $defaultParentId) == $main->id)>{{ $main->company_name }} ({{ $main->contractor_code }})</option>
              @endforeach
            </select>
          </div>
